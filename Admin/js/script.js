...
let tarrifSumm = document.querySelector('.tariff-summ');
...
function tarrifAdd(){
    ...
    const tarrifsCount =  document.querySelectorAll('.tarrifs-table tbody tr');
    const tarrifName = document.querySelector('.tarrifName');
    const licenseCount = document.querySelector('.license-count');
    const trackCount = document.querySelector('.track-count');
    let tarrifSumm = document.querySelector('.tariff-summ');
    //let tarrifSumEquals = (Number(licenseCount.value)*2000 + Number(trackCount.value)*2000).toLocaleString("ru-RU");
    tarrifSumm.value = tarrifSumm.value + "₸";
    
    tarrifsTable.innerHTML += `
        <tr id = "tarrif${tarrifsCount.length + 1}">
            <td>${tarrifName.value}</td>
            <td>${licenseCount.value}</td>
            <td>${trackCount.value}</td>
            <td>${tarrifSumm.value}</td>
            <td>
                <button title="Редактировать" class="tariff-edit" onclick="modal('.modal-tariff-edit')">
                ...
                <button title="Удалить" class="tariff-delette"  onclick="modal('.modal-tariff-delette');  numberTarrif(this)">
                    <svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function numberTarrif(count){
    elementNumber = '#' + count.closest('tr').id;
}
function deleteTarif(){
    document.querySelector(elementNumber).style.display = "none";
    closeModal();
}


function tarrifEdit(){
    const editTitle = document.querySelector(".modal-tariff-edit h3");
    const editName = document.querySelector(".tarif-edit-name");
    const editCount = document.querySelector(".license-edit-count");
    const editTrack = document.querySelector(".track-edit-count");
    const editSumm = document.querySelector(".tarif-edit-summ");
